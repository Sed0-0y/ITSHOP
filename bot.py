...
async def enter_price(message: types.Message, state: FSMContext):
    try:
        ...
        order_list.append({
            "name": product_name,
            "quantity": quantity,
            "weight": weight * quantity,  # Учитываем общий вес
            "price": price * quantity  # Учитываем общую стоимость
        })
...
async def confirm_order(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (user_id, data['name'], data['address'], data['phone'], data['email'], order_details, total_weight, total_cost, 0))
    conn.commit()
